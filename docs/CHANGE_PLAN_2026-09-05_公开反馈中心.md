...
| 旧系统 | 旧 feedback 入口收窄为客服/申诉性质，仅保留“账号问题、举报/纠纷、对局申诉”；Bug、建议、其他分类不再提供新入口。旧数据保留在库里，玩家与 staff 常规列表均不显示旧 bug/suggestion 线程，仅管理控制台可审计。不迁移到新站。 |
...
- 旧“反馈”按钮及其弹窗保留但语义收窄为“客服/申诉”：分类只显示 账号问题、举报/纠纷、对局申诉；Bug、建议、其他选项从前端隐藏；
...
- 新增逻辑层过滤：常规玩家/客服列表与 staff 查看列表不返回 `category IN ('bug','suggestion')` 的线程；
