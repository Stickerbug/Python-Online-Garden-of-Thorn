...
CARD_I18N = {
    ...
    'Yggdrasil': {
        ...
        'effect': _t('回复目标20H；自身受到致命伤害时将H设为5，抽3张牌，清除所有效果，无敌直到下回合结束，放逐此牌；可以选择一个阵亡玩家复活，并触发此效果', 'Heal the target for 20H; when you take lethal damage, set your H to 5, draw 3 cards, clear all effects, become invincible until the end of your next turn, and exile this card; you may choose a defeated player to revive and trigger this effect', 'Soigne la cible de 20H ; lorsque vous subissez des dégâts mortels, fixe votre H à 5, pioche 3 cartes, nettoie tous les effets, vous rend invincible jusqu’à la fin de votre prochain tour et exile cette carte ; vous pouvez choisir un joueur vaincu pour le ranimer et déclencher cet effet', '+20H; ao sofrer dano letal, se estiver na mão, limpa seus efeitos, define a vida como 5, fica invencível neste turno e exila esta carta', '+20H; при смертельном уроне, если карта в руке, очистить ваши эффекты, установить здоровье на 5, стать неуязвимым на этот ход и изгнать эту карту', '対象を20H回復する。自分が致命ダメージを受ける時、Hを5にし、カードを3枚引き、全ての効果を解除し、次の自分のターン終了まで無敵になり、このカードを放逐する。倒れたプレイヤーを選んで復活させ、この効果を発動できる'),
    },
    'Corruption': {
