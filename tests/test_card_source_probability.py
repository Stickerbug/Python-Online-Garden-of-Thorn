...
class CardSourceProbabilityUiContractTests(unittest.TestCase):
    def test_card_api_and_shared_term_panel_expose_metadata(self):
        ...
        self.assertIn("let galleryDataLoadedKey = '';", game_source)
        self.assertIn('function currentGalleryDataKey()', game_source)
        self.assertIn('galleryDataLoadedKey === queryKey', game_source)
        self.assertIn('if (queryKey !== currentGalleryDataKey()) return { cards, events, stale: true };', game_source)
        self.assertIn('function invalidateGalleryData()', game_source)
        self.assertIn('gallerySelectedModKeys = null;', game_source)
        self.assertIn("if (getVisibleViewId() === 'view-card-gallery')", game_source)
        self.assertIn('await ensureGalleryDataLoaded({ force: true });', game_source)
